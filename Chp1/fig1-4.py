...
if __name__ == '__main__':
    ...
    #Note lambda should actually be -infinity but -50 is good enough
    lmbda = np.exp(-100)
    X_train, T_train = generateData(1, 10, 0.3) #Generate training set
    X_test, T_test = generateData(1, 100, 0) #Generate test set

    X0 = np.linspace(0, 1, 100)
    M = np.array([[1,2],[4,10]]) #Number of basis
    #Interate of all basis/subplots

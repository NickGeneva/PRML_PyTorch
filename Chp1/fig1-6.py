...
if __name__ == '__main__':
    ...
    f.suptitle('Figure 1.6, pg. 9', fontsize=14)
    
    lmbda = np.exp(-50)
    X_test, T_test = generateData(1, 100, 0) #Generate test data

    X0 = np.linspace(0, 1, 100)
    N = np.array([15, 100]) #Number of training points
    
    for n, ax0 in enumerate(ax):
        X_train, T_train = generateData(1, N[n], 0.3)
        lsr = LeastSquaresReg(10,lmbda)

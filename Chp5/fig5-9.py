...
class simpleNN():
    def __init__(self, D_in, D_out, H, learning_rate):
        ...
        Args:
            D_in (Vaiable) = Number of input parameters
            D_out (Variable) = Number of output parameters
            H (Variables) = Number of hidden paramters
        """
        ...
        self.loss_fn = th.nn.L1Loss(size_average=False)
    ...
    def y_pred(self, x_train):
        """
        Get the prediction of the NN for a given set of points
        Args:
            X_train (th.DoubleTensor): [N x D_in] column matrix of training inputs
        Returns:
            y_pred (th.DoubleTensor): [N x D_out]
        """
...
if __name__ == '__main__':
    ...
    for idx, H in enumerate(hidden_units):
        ...
        y_pred = sNN.y_pred(X_test.expand(X_test.size(0),D_in))
